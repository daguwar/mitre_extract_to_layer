...
def extract_techniques_url(url: str, pattern: str) -> list:
    """Extract technique IDs from a URL."""
...
def extract_techniques_pdf(file: str, pattern: str) -> list:
    """Extract technique IDs from a PDF file."""
    try:
        import fitz
...
def write_layer_file(layer: dict, output_file: str) -> None:
    """Write the ATT&CK layer to a JSON file."""
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(layer, f, indent=2)


def print_counts(technique_counts: Counter) -> None:
    """Print the technique counts to the console."""
    sorted_counts = dict(
        sorted(
            technique_counts.items(),
            key=lambda item: item[1],
    ...
    for technique_id, value in sorted_counts.items():
        print(
            "Technique ID: "
            f"{technique_id.ljust(10)} Incidence: {str(value).ljust(4)}"
        )

    print(
        "Unique Number of MITRE ATT&CK Techniques found: "
        f"{total_techniques}.\n"
        "Total Instances of MITRE ATT&CK Techniques found: "
        f"{total_sum}"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract MITRE ATT&CK techniques and optionally build an ATT&CK Navigator layer."
    )
    parser.add_argument(
        "--text",
        help="Provide path for text based file to extract techniques"
    )
    parser.add_argument(
        "--url",
        help="Provide URL for web based report to extract techniques"
    )
    parser.add_argument(
        "--pdf",
        help="Provide path for PDF file to extract techniques"
    )
    parser.add_argument(
        "--res",
        default="t",
    ...
    parser.add_argument(
        "--output-layer",
        help="Write an ATT&CK Navigator layer JSON file"
    )
    parser.add_argument(
        "--layer-name",
        default="Extracted ATT&CK Techniques",
        help="Name for the generated ATT&CK Navigator layer"
    )
    parser.add_argument(
        "--layer-description",
        default="Layer generated from extracted ATT&CK technique frequency.",
        help="Description for the generated ATT&CK Navigator layer"
    )
    parser.add_argument(
        "--attack-version",
        default="19",
        help="MITRE ATT&CK version used in the layer versions object. Defaults to 19"
    )
    ...
    if args.url:
        ...
        print(
            "No valid arguments provided. Please use --text, --url, or --pdf."
        )
        sys.exit(1)
